...
export default function ModulePage() {
    ...
    return <main className="staff-shell learner-shell">
        <div className="staff-content learner-content">
            ...
            <section className="staff-page-heading learner-module-heading"><div><p className="staff-eyebrow">{t('module_number')} {module.position}</p><h1>{module.title}</h1><p>{module.description}</p></div><Badge className="staff-level">{module.completed ? t('module_completed') : `${progressPercent}%`}</Badge></section>
            {videoUrl ? <div className="player-frame learner-player-frame">
                ...
                <div className="learner-media-options">
                    {videoTracks.length > 1 && <label>{locale === 'fr' ? 'Langue de la vidéo' : 'Video language'}<Select value={selectedVideoLanguage} items={languageOptions(videoTracks.map((track) => track.language))} onChange={(value) => { if (typeof value === 'string' && videoRef.current) { pendingVideoPosition.current = { time: videoRef.current.currentTime, playing: !videoRef.current.paused }; window.localStorage.setItem(`synclearn_video_${moduleId}`, value); setVideoLanguage(value); } }} /></label>}
